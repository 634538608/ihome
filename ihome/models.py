...
class User(BaseModel, db.Model):
    ...
    @property
    def password(self):
        raise AttributeError("不允许读取操作")

    @password.setter
    def password(self, value):
        self.password_hash = generate_password_hash(value)  # 加密后存入数据库

    def check_password(self, value):
        return check_password_hash(self.password_hash, value)  # 以相同方式加密后对比
...
class House(BaseModel, db.Model):
    ...
    def to_dict(self):
        ...
        data = {
            'hid': self.id,
            'htitle': self.title,
            "harea_name": self.area.name,
            'hprice': self.price,
            'hindex_image_url': constants.QINIU_IMG_PREFIX + self.index_image_url,
            'hupdate_time': None
    ...
    def to_full_dict(self):
        ...
        house_dict = {
            ...
            "user_avatar": constants.QINIU_IMG_PREFIX + self.user.avatar_url if self.user.avatar_url else "",
            "title": self.title,
            "price": self.price,
        ...
        for image in self.images:
            img_urls.append(constants.QINIU_IMG_PREFIX + image.url)
...
class Order(BaseModel, db.Model):
    ...
    def to_dict(self):
        ...
        order_dict = {
            ...
            "img_url": constants.QINIU_IMG_PREFIX + self.house.index_image_url if self.house.index_image_url else "",
